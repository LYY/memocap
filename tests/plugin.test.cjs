...
test("plugin run invokes the packaged launcher without PATH", async (context) => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "memocap-plugin-"));
  context.after(() => fs.rmSync(tempDir, { recursive: true, force: true }));
  const binary = path.join(tempDir, "memocap-test.cjs");
  fs.writeFileSync(binary, `#!${process.execPath}\nprocess.stdout.write("plugin output\\n");`, {
    mode: 0o755,
  });

  const originalBinary = process.env.MEMOCAP_BINARY;
  const originalPath = process.env.PATH;
  process.env.MEMOCAP_BINARY = binary;
  process.env.PATH = tempDir;
  try {
    const { run } = await import(`${pathToFileURL(pluginPath).href}?test=${Date.now()}`);

    assert.equal(run(["recall", "memory"]), "plugin output\n");
  } finally {
